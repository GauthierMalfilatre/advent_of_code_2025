## Advent of Code day7 : Ex2 - Gauthier Malfilatre
...
def chillbro(lines: list[list[int]], x: int, y: int) -> int:
    ...
    if type(lines[y][x]) != int:
        return 0
    if y + 1 >= len(lines):
        return 0 
    value = lines[y][x]
    if lines[y + 1][x] == '^':
        lines[y + 1][x + 1] = value + (lines[y + 1][x + 1] if type(lines[y + 1][x + 1]) == int else 0)
        lines[y + 1][x - 1] = value + (lines[y + 1][x - 1] if type(lines[y + 1][x - 1]) == int else 0)
        return 0
    else:
        lines[y + 1][x] = value + (lines[y + 1][x] if type(lines[y + 1][x]) == int else 0)
        return 0
